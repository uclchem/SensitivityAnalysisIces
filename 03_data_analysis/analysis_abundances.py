...
plt.style.use(["science", "color_muted", "style_AA"])
...
if False:
    ...
    # )
    manager = DataManager(runsDir)
    models = manager.get_dataframes(T=T, nH=nH, zeta=zeta, radfield=UV, njobs=10)
    models = models.get_models_that_reach_desired_time(1e6)
    models.set_times(np.logspace(0, 6, num=100))
    models.create_standard_plot(
        spec, manager.parameters_RIN, manager.parameter_names, on_grain=True, njobs=10
    )
...
if False:
    ...
    fig, axs = plt.subplots(1, len(specs), sharex=True, sharey=True, figsize=(4.5, 3.0))
    for i, spec in enumerate(specs):
    ...
    if use_RIN:
        plt.savefig(f"correlationsWithHdiff__{int(temp)}K_RIN.pdf")
    else:
        plt.savefig(f"correlationsWithHdiff_{int(temp)}K_noRIN.pdf")
    plt.show()
...
if False:
    ...
    plt.savefig(
        f"BIGplot_{zeta}_{UV}_COMs.pdf", bbox_extra_artists=[leg, leg2, *suplabels]
    )
    plt.show()
...
if False:
    ...
    for i, spec in enumerate(species):
        axs[i].set_title(convertSpeciesToLegendLabel(spec), y=0.98)
