...
IMPLICIT NONE
    ...
    LOGICAL :: desorb,h2desorb,crdesorb,uvdesorb,chemdesorb,thermdesorb
    ...
    REAL(dp) :: freezeFactor,omega,grainArea,cion,h2dis,lastGasTemp,lastDustTemp=0.0D0
...
CONTAINS
    SUBROUTINE initializeChemistry(readAbunds)
        ...
        NEQ=nspec+1
        IF (ALLOCATED(abund)) DEALLOCATE(abund)
        ALLOCATE(abund(NEQ,points))
        ...
        ! WRITE(*,*) "Calculating desorption fractions of LHDES reactions"
        desorptionFractionsBare = 0.0D0
        desorptionFractionsFullCoverage = 0.0D0
        DO j = lhdesReacs(1), lhdesReacs(2)
            ! WRITE(*,*) "  LHDES reaction index:", j
            ! WRITE(*,*) "  ", specName(re1(j)), specName(re2(j)), specName(p1(j)), &
            !     specName(p2(j)), specName(p3(j))
        ...
        END DO
        ! WRITE(*,*) "Calculating desorption fractions of ERDES reactions"
    ...
    !the code integrates to the planned targetTime rather than a reduced one. If the counter reaches maxConsecutiveFailures, we end the code.
        ...
        DO WHILE((currentTime .lt. targetTime) .and. (loopCounter .lt. maxLoops)) 
            ...
            h2Col=h2ColToCell+0.5D0*abund(nh2,dstep)*density(dstep)*(cloudSize/real(points))
            coCol=coColToCell+0.5D0*abund(nco,dstep)*density(dstep)*(cloudSize/real(points))
            cCol=cColToCell+0.5D0*abund(nc,dstep)*density(dstep)*(cloudSize/real(points))
            ...
            bulkLayersReciprocal=MIN(1.0,NUM_SITES_PER_GRAIN/(GAS_DUST_DENSITY_RATIO*safeBulk)) ! Should this MIN be here?
            surfaceCoverage=bulkGainFromMantleBuildUp()

            IF ((.NOT. dustTemp(dstep) .eq. lastDustTemp) .OR. &
                (.NOT. gasTemp(dstep) .eq. lastGasTemp)) THEN
                CALL updateVdiffAndVdes(gasTemp(dstep), dustTemp(dstep), vdiff, vdes)
            END IF

            ! WRITE(*,*) "Calculating reaction rates"
            CALL calculateReactionRates

            !Integrate chemistry, and return fail if unrecoverable error was reached
            ! WRITE(*,*) "Integrating ODEs"
        ...
        IF (ABS(originalTargetTime- targetTime) .lt. 0.001D0*originalTargetTime) THEN
            failedIntegrationCounter=0
        ELSE
    ...
    SUBROUTINE F (NEQUATIONS, T, Y, YDOT)
        ...
        if (refractoryList(1) .gt. 0) safeBulk=safeBulk-SUM(Y(refractoryList))
