...
chemdesorb=.True. !Individually toggle non-thermal desorption due to chemical reactions.
...
diffToBindRatio=0.5 !Ratio of diffusion barrier to binding energy of all species
h2EncounterDesorption=.True. !Encounter desorption mechanism of Hincelin et al 2015 (H2 on H2)
hEncounterDesorption=.False. !Encounter desorption mechanism of Hincelin et al 2015 (H on H2)
EDEndothermicityFactor=0.0 !Account for endothermicity of moving off of H2O onto H2 by fraction of diff in binding energies
h2StickingCoeffByh2Coverage=.False. !Decrease sticking coeff of H2 by H2 coverage of surface
hStickingCoeffByh2Coverage=.False. !Decrease sticking coeff of H by H2 coverage of surface
HdiffusionBarrier=-1.0 !Diffusion barrier for atomic H on grain surface (K). 
!!This is later corrected to diffToBind*Ebind(#H) if no other value is input
useCustomDiffusionBarriers=.True. !Use custom diffusion barriers, instead of assuming they're a fraction of the binding energy
seperateDiffAndDesorbPrefactor=.True. !Calculate different prefactors for diffusion and desorption
useTSTprefactors=.False. !Calculate diffusion and desorption prefactors using TST. Otherwise, use Hasegawa-Herbst equation.
useCustomPrefactors=.False. !Use custom diffusion and desorption prefactors, instead of TST or Hasegawa-Herbst values.
useMinissaleIceChemdesEfficiency=.False. !Use Minissale 2016 efficiency for chemical desorption on ices. If False, use Fredon 2021
!
!## Input and Output
!|Parameter|Default Value |Description|
!| ----- | ------| ------ |
