...
CONTAINS
    ...
    IF ((density .lt. finalDens) .and. (freefall)) THEN
        ...
        &(8.4d-30*initialDens*((density/initialDens)**0.33-1.0))**0.5
    ELSE
        densdot=0.0
    ENDIF
    ...
    IF (density .lt. finalDens) THEN
        dByDndensdot=freefallFactor*8.4d-30*(density**3.0)*((9.0*((density/initialDens)**0.33))-8.0)
        dByDnDensdot=dByDnDensdot/(6.0*(((density**4.0)/initialDens)**(0.66)))
    ...
    SUBROUTINE ionizationDependency
        ...
        zeta=((10.0**zSum)/1.3d-17)* zetaScale
        ...
        IF (improvedH2CRPDissociation) THEN
            ...
            END DO
            h2CRPRate=(10.0**dissSum)*zetaScale
        END IF
