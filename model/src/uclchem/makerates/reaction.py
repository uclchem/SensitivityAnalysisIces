...
class Reaction:
    def __init__(self, inputRow, reaction_source=None):
        try:
            ...
            raise ValueError(
                f"Input for Reaction should be a list of length 13. The following reaction caused this error: {inputRow}"
            ) from error
        ...
        self.is_tunneling = self.get_reaction_type() in ["LH", "LHDES", "ER", "ERDES"]
        if self.is_tunneling and self._reduced_mass == 0.0:
            # If the reaction is tunneling based, and no reduced mass was supplied to it, try to predict it
    ...
    def predict_reduced_mass(self) -> None:
        ...
        for reac in self._reactants:
            if reac in reaction_types:
                continue
            specie = Species([reac] + [0] * (speciesNfields - 1), do_checks=False)
        ...
        for prod in self._products:
            if prod in "NAN":
                continue
            specie = Species([prod] + [0] * (speciesNfields - 1), do_checks=False)
            atoms = specie.find_constituents(quiet=True)
        ...
        if n_reacs == n_prods:
            ...
            # For one-product reaction, we can just use reduced mass of two species
            self.set_reduced_mass(naive_reduced_mass)
            logging.debug(
                f"Predicted reduced mass of one-product reaction '{self}' to be {self._reduced_mass} using naive equation"
            )
            # Addition reaction
    ...
    def generate_ode_bit(self, i: int, species_names: list, three_phase: bool):
        ...
        for species in self.get_reactants():
            if species in species_names:
                ...
                # Changed according to Garrod 2013, maybe correct math, maybe not?
...
class CoupledReaction(Reaction):
    ...
    def __init__(self, reaction: Reaction):
        try:
            ...
            self.set_source(reaction.get_source())
            self.set_reduced_mass(reaction.get_reduced_mass())
            self.partner = None
        except ValueError as err:
            raise ValueError(f"Error initializing CoupledReaction, {err}")
