...
from uclchem.utils import extend_lst_to_length

from .reaction import CoupledReaction, Reaction, reaction_types
from .species import Species, elementList, speciesNfields
...
class Network:
    ...
    def __init__(
        ...
        electron_specie = Species(extend_lst_to_length(["E-"], speciesNfields))
    ...
    def add_reactions(
        ...
        for reaction in reactions:
            ...
            for specie in species_not_present:
                ...
                self.add_species(
                    Species(extend_lst_to_length([specie, -1], speciesNfields))
                )
    ...
    def remove_reaction(self, reaction: Reaction) -> None:
        ...
        elif len(reaction_idx_dict_as_tuples) == 2 and (
            reaction_idx_dict_as_tuples[0][1].get_products()[:2][::-1]
            == reaction_idx_dict_as_tuples[1][1].get_products()[:2]
        ):
            # Two reactions form two the same two products, but one is for example desorbed, and the other is not.
            # Examples: #O + #H2O + LHDES -> #OH + OH, and #O + #H2O + LHDES -> OH + #OH
            for i in range(2):
                reac_idx, reac_value = reaction_idx_dict_as_tuples[i]
                logging.debug(f"Trying to remove index: {reac_idx}: {reac_value}")
                del self._reactions_dict[reac_idx]
    ...
    def add_bulk_species(self) -> None:
        ...
        new_species = []
        try:
            h2o_index = speciesNames.index("#H2O")
            h2o_binding_energy = self.get_species_list()[h2o_index].binding_energy
            h2o_diffusion_barrier = self.get_species_list()[h2o_index].diffusion_barrier
        except ValueError:
            ...
            if species.is_surface_species():
                if species.name.replace("#", "@") not in speciesNames:
                    ...
                    if new_spec.name in userSpecies:
                        ...
                        definedDiffusion = [
                            userSpec.diffusion_barrier
                            for userSpec in self.user_defined_bulk
                            if userSpec.name == new_spec.name
                        ]
                        new_spec.diffusion_barrier = definedDiffusion[0]
                    else:
                        new_spec.binding_energy = h2o_binding_energy
                        new_spec.diffusion_barrier = h2o_diffusion_barrier
                ...
                for products, alpha in species.get_freeze_products():
                    new_reactions.append(
                        Reaction(
                            ...
                            + products
                            + [alpha, 0.0, species.binding_energy, 0.0, 10000.0, 0]
                        )
                    )
                    ...
                    if products[0] not in self.get_species_list():
                        ...
                        new_species.append(
                            Species(
                                extend_lst_to_length(
                                    [products[0], species.mass, species.binding_energy],
                                    speciesNfields,
                                )
                            )
                        )
                ...
                for reacType in desorb_reacs:
                    new_reactions.append(
                        Reaction(
                            ...
                            + [1, 0, species.binding_energy, 0.0, 10000.0, 0]
                        )
                    )
        ...
        for reaction in lh_reactions:
            ...
            if reaction.get_reactants()[0] + "*" in [
                ...
                new_reac_A_list = (
                    ...
                    + [reaction.get_alpha(), 0, 0, 0, 10000, 0]
                )
                new_reac_B_list = [
                    reaction.get_reactants()[0],
                    reaction.get_reactants()[1] + "*",
    ...
    def get_tunneling_reactions(self) -> list[Reaction]:
        reactions_tunneling = [
            reaction for reaction in self.get_reaction_list() if reaction.is_tunneling
        ]
        return reactions_tunneling
        ...
        if len(reaction_idx_dict_as_tuples) == 1:
            ...
            if not isinstance(reaction, CoupledReaction):
                ...
                for k, v in self._reactions_dict.items():
                    ...
                    if v.get_partner() == reaction:
                        logging.debug(
                            f"Coupled reaction {v} will also have its energy barrier changed"
                        )
                        to_change.append(v)
                if to_change:
                    [
            ...
            logging.warning(
                f"The reaction {reaction} is not present in the reaction set, so cannot change its barrier"
            )
        elif len(reaction_idx_dict_as_tuples) == 2 and (
            reaction_idx_dict_as_tuples[0][1].get_products()[:2][::-1]
            == reaction_idx_dict_as_tuples[1][1].get_products()[:2]
            ...
            for i in range(2):
                ...
                if not isinstance(reaction, CoupledReaction):
                    ...
                    if to_pop:
                        [
                            self.change_reaction_barrier(coupled_reac, barrier)
                            for coupled_reac in to_pop
                        ]
        elif len(reaction_idx_dict_as_tuples) > 1:
            raise (
                RuntimeError(
                    f"found more than one indices for the reaction {reaction}."
                )
            )
    ...
    def change_diffusion_barrier(
        self, specie: str, new_diffusion_barrier: float
    ) -> None:
        all_species = self.get_species_list()
        all_species_names = [specie.name for specie in all_species]
        if specie not in all_species_names:
            error = f"Specie {specie} was not found in the network while attempting to change its binding energy."
            raise ValueError(error)
        old_bulk_H2O_diffusion_barrier = all_species[all_species_names.index("@H2O")]
        old_bulk_H2O_diffusion_barrier = (
            old_bulk_H2O_diffusion_barrier.diffusion_barrier
        )
        if specie == "@H2O":
            # If specie is bulk H2O, we need to change diffusion barriers of all other bulk species,
            # as the diffusion is limited by diffusion of bulk H2O. (Ghesquiere 2015)
            for specie_in_network in all_species:
                if (
                    "@" in specie_in_network.get_name()
                    and specie_in_network.diffusion_barrier
                    == old_bulk_H2O_diffusion_barrier
                ):
                    # If the specie had a different bulk binding energy, do not change it, as it was user specified.
                    specie_in_network.diffusion_barrier = new_diffusion_barrier
            return
        if "@" in specie:
            if (
                all_species[all_species_names.index(specie)].diffusion_barrier
                == old_bulk_H2O_diffusion_barrier
            ):
                # If the bulk species has the same binding energy as bulk H2O,
                # but we are trying to change it directly, give user a warning.
                print(
                    f"WARNING: ATTEMPTING TO CHANGE DIFFUSION BARRIER OF BULK SPECIE {specie} THAT WAS PREVIOUSLY @H2O DIFFUSION LIMITED"
                )
        for specie_in_network in all_species:
            if specie_in_network.get_name() == specie:
                specie_in_network.diffusion_barrier = new_diffusion_barrier
    ...
    def change_prefactor(
        self, specie: str, new_prefactor: float, prefactor_type: str = None
    ) -> None:
        if prefactor_type is None or not isinstance(prefactor_type, str):
            msg = "prefactor_type argument of change_prefactor should be a string, but was type"
            msg += f" {type(prefactor_type)}."
            raise TypeError(msg)

        prefactor_type = prefactor_type.lower()
        if prefactor_type not in ["desorption", "diffusion"]:
            msg = f"prefactor_type should be either 'desorption' or 'diffusion', but was {prefactortype}."
        ...
        if specie not in all_species_names:
            error = f"Specie {specie} was not found in the network while attempting to change its binding energy."
            raise ValueError(error)
        spec_index = all_species_names.index(specie)

        if prefactor_type == "diffusion":
            all_species[spec_index].vdiff = new_prefactor
        elif prefactor_type == "desorption":
            all_species[spec_index].vdes = new_prefactor
        return
    ...
    def branching_ratios_checks(self) -> None:
        ...
        branching_reactions = {}
        for i, reaction in enumerate(self.get_reaction_list()):
            if reaction.get_reaction_type() in ["LH"]:
                ...
                if reactant_string in branching_reactions:
                    ...
                    branching_reactions[reactant_string] = reaction.get_alpha()
        if all(branching_reactions.values()) == 1.0:
            return
        logging.warning(
            "Some of the branching ratios do not sum to 1.0, correcting those that do not"
        )
        for i, reaction in enumerate(self.get_reaction_list()):
            if not reaction.get_reaction_type() in ["LH"]:
                continue
            reactant_string = ",".join(reaction.get_reactants())
            ...
            if (
                ...
                if branching_reactions[reactant_string] != 0.0:
                    if branching_reactions[reactant_string] < 0.99:
                        ...
                        continue
                    new_alpha = (
                        new_reaction.get_alpha() / branching_reactions[reactant_string]
                    )
                    logging.warning(
                    ...
                    self.remove_reaction(reaction)
        # For two-product LHDES reactions:
        # If we have LHDES reactions, we need to check if the two products are the same, but one is desorbed and the other is not
        # Then somehow, we need to take into account only
        for i, reaction in enumerate(self.get_reaction_list()):
            if not reaction.get_reaction_type() in ["LHDES"]:
                continue
            reactant_string = ",".join(reaction.get_reactants()).replace("LHDES", "LH")
            # Check if we need to correct the branching ratio (smaller than 0.98 is allowed)
            if (
                reactant_string in branching_reactions
                and branching_reactions[reactant_string] != 1.0
                ...
                if branching_reactions[reactant_string] != 0.0:
                    if branching_reactions[reactant_string] < 0.99:
                        ...
                        continue
                    new_reaction = deepcopy(reaction)
                    reactionPartner = reaction.get_parter()
                    while isinstance(reactionPartner, CoupledReaction):
                        reactionPartner = reactionPartner.get_partner()
                    new_alpha = reactionPartner.get_alpha()
    ...
    def find_species_react_with_itself(self) -> list[Species]:
        reaction_list = self.get_reaction_list()
        species_react_with_themselves = []
        for i, spec in enumerate(self.get_species_list()):
            for reaction in reaction_list:
                reactants = reaction.get_reactants()
                if reactants[0] == reactants[1] == spec:
                    if i + 1 not in species_react_with_themselves:
                        species_react_with_themselves.append(i + 1)
        return list(species_react_with_themselves)
