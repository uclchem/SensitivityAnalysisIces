        ...
        with open(file_name, "r") as f:
            ...
            for row in reader:
                if row[0].startswith("#") or row[0].startswith("!"):
                    continue
                reaction_row = row[2:4] + [""] + row[4:8] + row[9:14] + [""] + row[14:]
...
def write_evap_lists(network_file, species_list: list[Species]) -> int:
    ...
    for i, species in enumerate(species_list):
        if species.name[0] == "#":
            ...
            customVdesList.append(species.vdes)
            diffusion_barriersList.append(species.diffusion_barrier)
            customVdiffList.append(species.vdiff)
    ...
    network_file.write(
        array_to_string("bindingEnergy", binding_energyList, type="float")
    )
    ...
    network_file.write(array_to_string("moleculeIsLinear", isLinears, type="bool"))
    network_file.write(
...
def write_network_file(file_name: Path, network: Network):
    ...
    atoms = []
    masses = []
    canReactWithItself = network.find_species_react_with_itself()
    for species in species_list:
    ...
    openFile.write(
        array_to_string("    speciesReactWithItself", canReactWithItself, type="int")
    )
    ...
    for reaction in reaction_list:
        if not reaction.get_reaction_type() in ["ERDES"]:
            continue
        partner = reaction.get_partner()
        reacIndex = reaction_list.index(partner) + 1
...
def array_to_string(
    ...
    Args:
        ...
        type (str, optional): The array's type. Must be one of "int","float", "string", "bool" or "array".Defaults to "int".
    ...
    """
    if parameter:
        outString = ", PARAMETER :: " + name
        if not type == "array":
            outString += " ({0})=(/".format(len(array))
        else:
            outString += " ({0},{1})=reshape((/(/".format(len(array[0]), len(array))
    else:
        if not type == "array":
            outString = " :: " + name + " ({0})=(/".format(len(array))
        else:
            outString += " ({0},{1})=(/".format(len(array[0]), len(array))
    if type == "int":
        ...
        outString = "REAL(dp)" + outString
        for subarr in array:
            outString += ",".join([str(val) for val in subarr])
            outString += "/),(/"
        outString = outString[:-3] + "/), (/{0},{1}/)".format(len(array[0]), len(array))
    else:
