...
from uclchem.utils import _product_count, _reactant_count
...
def get_change_df(
    ...
    if "#" in species or "@" in species:
        ...
        msg += "USE FUNCTION uclchem.analysis._get_rates_change WITH SPECIES CONTAINING # OR @ TO INDICATE SURFACE OF BULK."
...
def analysis_solid_phase(species_name, result_df):
    if species_name[0] == "$":
    ...
    surf_columns, bulk_columns = surf_df.columns, bulk_df.columns
    for column in surf_columns:
        if not "->" in column:
            bulk_df.drop(
                columns=column, inplace=True
            ...
            continue
        # if column in bulk_columns:  # and (
        #     # (
        #     #     _reactant_count("#" + species_name, column) == 1
        #     #     and _product_count("@" + species_name, column) == 1
        #     # )
        #     # or (
        #     #     _reactant_count("@" + species_name, column) == 1
        #     #     and _product_count("#" + species_name, column) == 1
        ...
        #     # which means that either surface is lost and bulk forms, or the other way, and so we drop this column
        #     # from both dfs since it has no effect on the ice overall.
        #     surf_df.drop(columns=column, inplace=True)
        #     bulk_df.drop(columns=column, inplace=True)
    # Maybe TODO:
    # Make it such that the columns of the same reactions (but surf and bulk versions) are added
    # such that we have a single reaction rate in the ice, and not seperate surf and bulk reaction rates.
    return pd.concat([surf_df, bulk_df], axis=1)


def getNetChange(rate_df):
    reactionColumns = [col for col in rate_df.columns if "->" in col]
    return rate_df[reactionColumns].sum(axis=1)
...
def analysis(
    ...
    physicalCondColumns = result_df.columns[: list(result_df.columns).index("H")]
    nPhysicalCondColumns = len(physicalCondColumns)
    if species_name[0] == "#":
        surftransfer_reacs = [
    ...
    for i, row in result_df.iterrows():
        ...
        if transfer <= 0:
            ...
            if species_name[0] == "#":
                ...
                change_reacs.append(surftransfer_reacs[0])
                changes = np.append(changes, transfer)
        # changes = np.append(changes, transfer) # Adding transfer twice?
        if return_dataframe:
            if i == 0:
                columns = list(physicalCondColumns) + list(change_reacs)
                df = pd.DataFrame(columns=columns)
            df.loc[i] = np.concatenate((row[physicalCondColumns].values, changes))
        else:
...
def _get_rates_of_change(
    ...
    safeBulk = np.max([1.0e-30, row["BULK"]])
    ratioSurfaceToBulk = np.min([1.0, safeMantle / safeBulk])
    for i, reaction in enumerate(reactions):
        reaction_instance = Reaction([*reaction, 0, 0, 0, 0, 0, 0])
        change = rates[i]

        if "FREEZE" in reaction:
            print(change)

        reactants = list(reaction[:3])
        products = list(reaction[3:])

        # Counting the same as Reaction.body_count
        for body in range(reaction_instance.body_count):
        ...
        for reactant in reactants:
            if reactant in speciesList:
                ...
                change = change * ratioSurfaceToBulk
        ...
        # print(reaction, products.count(species) - reactants.count(species))
        changes.append((products.count(species) - reactants.count(species)) * change)

    # This messes up if we do return_dataframe, I do not understand why.
    # Sets some entire columns incorrectly to 0.
